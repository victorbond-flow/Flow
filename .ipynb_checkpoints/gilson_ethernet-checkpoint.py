...
# This function creates the XML command that tells the Gilson what to do. You provide parameters as a python dictionary, and this turns them into parameter tags
...
# Creates and sends a local command to the Gilson, then returns the instrument's response
...
# Similar to the above, except now the command is executed instantly - skipping the queue
...
# Sends an initial admin command to the Gilson to confirm/establish connection
...
# Closes the active session and cleans up the connection
    def close(self):
